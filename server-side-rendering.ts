...
export function addServerSideRendering(app: express.Express, handlebarsEngine: any) {
    const devEnv = app.get('env') === 'development';
    const viewsDir = app.get('views');
    // Serve the views directory so we can include the component's script on the page
    app.use(express.static(viewsDir));
    const ssrEngine = function (viewFile: string, options: any, callback: (...args: any[]) => void) {
        // Allow specifying the template by the name of the source file or the transpiled file

        // const viewFilePath = viewFile.replace(/\.tsx$/, '.js')

        const relativeViewFilePath = path.join('/', path.relative(viewsDir, viewFile));
        ...
        try {
            ...
            const layoutTemplate = path.join(viewsDir, 'templates', 'react-layout.hbs');
            ...
            if (e && e.code === 'MODULE_NOT_FOUND') {
                return callback(new Error(`Could not load the component ${path.basename(viewFile)}. Did you run \`npm build\` to compile your tsx files?`));
            }
            return callback(e);
        } finally {
            sheet && sheet.seal();
